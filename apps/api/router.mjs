...
import { handleQueueEvents } from "./handlers/events.mjs";
import { handleHealth, handleReady } from "./handlers/health.mjs";
import {
  handleCreateSession,
  handleDeleteSession,
  handleGetSession,
...
export async function handleApiRequest(incoming) {
  ...
  if (request.method === "GET" && pathname === "/health/ready") {
    const result = await handleReady();
    return withCors(request, jsonResponse(result.status, result.body));
  }
  ...
  try {
    if (request.method === "GET" && pathname === "/v1/events/queue") {
      const controller = new AbortController();
      incoming.on?.("close", () => controller.abort());
      const streamResponse = await handleQueueEvents(auth.workspaceId, controller.signal);
    ...
    if (request.method === "GET" && pathname === "/v1/projects") {
      const result = await handleListProjects(auth.workspaceId);
      return withCors(request, jsonResponse(result.status, result.body));
    }

    if (request.method === "GET" && pathname.startsWith("/v1/projects/")) {
      const projectId = pathname.slice("/v1/projects/".length);
      const result = await handleGetProject(auth.workspaceId, projectId);
      return withCors(request, jsonResponse(result.status, result.body));
    }
    ...
    if (request.method === "POST" && pathname === "/v1/projects") {
      ...
      const input = parseCreateProjectInput(parsed.value);
      if (input.error) {
        return withCors(request, jsonResponse(400, { error: input.error }));
      }
    ...
    if (request.method === "PATCH" && pathname.startsWith("/v1/projects/")) {
      const projectId = pathname.slice("/v1/projects/".length);
      const parsed = await readJsonBody(incoming);
      if (parsed.error) {
        return withCors(request, jsonResponse(parsed.error.status, { error: parsed.error.message }));
    ...
    if (request.method === "DELETE" && pathname.startsWith("/v1/projects/")) {
      const projectId = pathname.slice("/v1/projects/".length);
      const result = await handleArchiveProject(
        auth.workspaceId,
        auth.actor,
        projectId,
        idempotencyKey,
