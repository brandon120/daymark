...
- Project CRUD with optimistic versioning, soft archive, and idempotent mutations.
- Projects workspace UI with create, edit, and archive forms.
- Realtime work-queue updates through `GET /v1/events/queue` (SSE).
- Production startup validation, readiness checks, pool limits, and security headers.
- Audit events for priority toggles, active-project changes, Bee context changes,
  project create/update/archive, coding-task enqueue, and coding-task advancement.
...
1. richer authentication than a single bearer token;
2. integration tests in CI with PostgreSQL;
3. tracing and basic budget hooks;
4. expand realtime events beyond the work queue.
