...
test("client mutations require caller-supplied idempotency keys", async () => {
  ...
  assert.match(source, /createProject\(input, idempotencyKey\)/);
  assert.match(source, /updateProject\(projectId, input, idempotencyKey\)/);
  assert.match(source, /archiveProject\(projectId, idempotencyKey\)/);
});
